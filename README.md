...
- `notes/` — Per-customer live intel: confirmed WhatsApp updates and rep notes from the "Needs attention" queue (runtime state; `SEARAY_NOTES_DIR` to relocate)
...
The top 5 customers to work on now (`utils/attention.py`, thresholds in `constants/attention.py`):

1. **Late for an order** — days since their last order is ≥1.5× their usual gap (the average gap between their order dates, needs 3+ orders) and at least 14 days past it
2. **No recent note** — everyone else
3. **Noted in the last 30 days** — the back of the line, oldest note first

Biggest 2-year spend first within 1 and 2. Saving a note (or confirming a WhatsApp update) sends a customer to the back of the line and the next one pops up; the note also shows under "What's going on" on their visit page.
...
| `GET` | `/api/attention` | Needs-attention queue: `{ queue: [card + why/reason/last_note], overdue, noted_recently, snooze_days }` |
| `POST` | `/api/attention/<code>/note` | Body: `{ "text" }` → save a rep note, returns the refreshed queue |
| `GET` | `/api/customer/<code>` | Customer detail and current three actions |
| `POST` | `/api/intel/summarise` | Body: `{ "text" }` (a WhatsApp dump) → `{ proposals: [update] }`, nothing saved. 503 if `ANTHROPIC_API_KEY` is unset |
| `GET` | `/api/intel/<code>` | Saved live updates for a customer |
| `POST` | `/api/intel/<code>` | Save one update (body = a proposal) → refreshed prep |
| `POST` | `/api/intel/<code>/delete` | Body: `{ "id" }` → refreshed prep |
...
- Learned preferences are saved locally in `feedback/`, and notes in `notes/`; back up those folders if you want to preserve rep feedback between sessions
- **On Render**, the service's own disk is wiped on every deploy. Attach a persistent disk (e.g. mounted at `/var/data`) and set `SEARAY_NOTES_DIR=/var/data/notes` and `SEARAY_FEEDBACK_DIR=/var/data/feedback` on the engine service, or notes — and so the attention queue order — reset on each push
